...
interface UnitStatusProps {
  entity: BattleEntity;
  isOpponent?: boolean;
  intention?: string | null; // For AI next move
}
  ...
  return (
    <div className={`flex items-center gap-4 ${isOpponent ? 'flex-row-reverse text-right' : 'flex-row'}`}>
      ...
      <div className={`flex flex-col ${isOpponent ? 'items-end' : 'items-start'}`}>
        <h2 className="text-2xl font-display font-bold text-white uppercase tracking-widest">{entity.name}</h2>
        
        {/* Stats */}
        <div className="flex items-center gap-3 mt-1 text-sm font-mono text-slate-400">
            {intention && isOpponent && (
                <div className="flex items-center gap-1 text-yellow-500 animate-pulse bg-yellow-500/10 px-2 py-0.5 rounded">
                    <Activity size={14} />
